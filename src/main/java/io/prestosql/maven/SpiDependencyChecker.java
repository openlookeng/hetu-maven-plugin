...
{
    @Parameter(defaultValue = "io.hetu.core")
        ...
        for (Artifact artifact : project.getArtifacts()) {
            ...
            if (spiDependencies.contains(name)) {
                if (!"jar".equals(artifact.getType())) {
                    throw new MojoExecutionException(format("%n%nHetu plugin dependency %s must have type 'jar'.", name));
                }
                if (artifact.getClassifier() != null) {
                    throw new MojoExecutionException(format("%n%nHetu plugin dependency %s must not have a classifier.", name));
                }
                if (!"provided".equals(artifact.getScope())) {
                    throw new MojoExecutionException(format("%n%nHetu plugin dependency %s must have scope 'provided'. It is part of the SPI and will be provided at runtime.", name));
                }
            }
            else if ("provided".equals(artifact.getScope()) && !allowedProvidedDependencies.contains(name)) {
                throw new MojoExecutionException(format("%n%nHetu plugin dependency %s must not have scope 'provided'. It is not part of the SPI and will not be available at runtime.", name));
            }
        }
    }
            ...
            throws MojoExecutionException
    {
        for (Artifact artifact : project.getArtifacts()) {
            if (isSpiArtifact(artifact)) {
                if (!"provided".equals(artifact.getScope())) {
                    throw new MojoExecutionException(format("%n%nHetu plugin dependency %s must have scope 'provided'.", spiName()));
                }
                return artifact;
            }
        }
        throw new MojoExecutionException(format("%n%nHetu plugin must depend on %s.", spiName()));
    }
